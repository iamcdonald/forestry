...
describe('TreeNode', function () {
	describe('constructor', function () {
		it('returns correctly populated TreeNode object', function () {
			var tn = new TreeNode('id', 'value');

			assert.equal(tn.id, 'id');
			assert.equal(tn.value, 'value');
			assert.equal(tn.parent, null);
			assert(tn.children instanceof Array);
			assert.equal(tn.children.length, 0);
		});
	...
	describe('prototype methods', function () {
		...
		beforeEach(function () {
			root = new TreeNode('a');
			root.addChild(new TreeNode('a/1').addChild(new TreeNode('a/1/i')));
			root.addChild(new TreeNode('a/2'));
		});	

		describe('addChild', function () {

			it('adds child and sets parent properly', function () {
				root.addChild(new TreeNode('a/3', 'val'));
